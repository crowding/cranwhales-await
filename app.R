...
server <- function(input, output, session) {
  ...
  whale_downloads <- reactive(async({
    await(promise_all(data = data(), whales = whales())) |>
      with({
        data |>
  ...
  output$total_size <- renderValueBox(async({
    await(data()) |>
      pull(size) |>
  ...
  output$total_count <- renderValueBox(async({
    await(data()) |>
      nrow() |>
      format(big.mark = ",") |>
  ...
  output$total_uniques <- renderValueBox(async({
    await(data()) |>
      pull(package) |>
      unique() |>
      length() |>
  ...
  output$all_hour <- renderPlot(async({
    await(promise_all(data = data(), whales = whales())) |>
      with({
  ...
  output$downloaders_hour <- renderPlot(async({
    await(whale_downloads()) |>
